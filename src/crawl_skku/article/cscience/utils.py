...
def parse_post_list(html: str) -> list[ArticlePostListItem]:
    ...
    if not items and list_container.find_all("li", recursive=False):
        raise SkkuParseError("Could not parse any SKKU CScience post rows")

    return items


def parse_post_detail(
    html: str, board_id: int, item_id: str
) -> CscienceArticlePostDetail:
    soup = BeautifulSoup(html, "html.parser")
    box = soup.select_one(".board-view-box")
    if box is None:
        raise SkkuParseError("Could not find SKKU CScience post detail")

    title_wrap = box.select_one(".board-view-title-wrap")
    title_tag = title_wrap.select_one("h4") if title_wrap else None
    if title_tag is None:
        raise SkkuParseError("Could not parse SKKU CScience post title")
    ...
    return CscienceArticlePostDetail(
        key=CscienceArticleKey(
            type="cscience_article",
            board_id=board_id,
            item_id=item_id,
        ),
        category=category,
        ...
        date=post_date,
        author=author,
        url=constants.build_detail_url(board_id, item_id),
...
def _parse_key(href: str) -> CscienceArticleKey | None:
    query = parse_qs(urlparse(href).query)
    board_ids = query.get("viewBoardId")
    item_ids = query.get("itemId")
    if not board_ids or not item_ids or not item_ids[0]:
        return None

    try:
        return CscienceArticleKey(
            type="cscience_article",
...
def _parse_date_from_info(info_texts: list[str]) -> tuple[date, int]:
    ...
    raise SkkuParseError("Could not parse SKKU CScience post date")
...
def _parse_list_author(info_texts: list[str], date_index: int) -> str | None:
    ...
    if not author or author.startswith(("No.", "공지")):
        return None
...
def _parse_description(box: Tag) -> str:
    content = _find_description_content(box)
    if content is None:
        raise SkkuParseError("Could not find SKKU CScience post description")
    ...
    if not description:
        raise SkkuParseError("Could not parse SKKU CScience post description")
