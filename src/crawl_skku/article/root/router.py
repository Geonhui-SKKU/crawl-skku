...
from crawl_skku.article.root.dependencies import RootArticlePostServiceDep
...
router = APIRouter(tags=["SKKU root posts"])


@router.get(
    "/posts",
    summary="List SKKU general posts",
)
async def list_posts(
    service: RootArticlePostServiceDep,
...
@router.get(
    "/posts/{article_no}",
    summary="Get an SKKU general post",
)
async def get_post(
    service: RootArticlePostServiceDep,
...
def _bad_gateway(exc: SkkuCrawlerError) -> HTTPException:
    return HTTPException(
        ...
        detail=str(exc) or "Failed to read SKKU upstream page",
    )
