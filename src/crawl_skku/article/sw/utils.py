...
from crawl_skku.article.sw import constants
...
def parse_post_list(html: str) -> list[SkkuArticlePostListItem]:
    ...
    if list_container is None:
        raise SkkuParseError("Could not find SKKU SW post list")
    ...
    if not items and list_container.find_all("li", recursive=False):
        raise SkkuParseError("Could not parse any SKKU SW post rows")
...
def parse_post_detail(html: str, article_no: int) -> SkkuArticlePostDetail:
    ...
    if box is None:
        raise SkkuParseError("Could not find SKKU SW post detail")

    title_wrap = box.select_one(".board-view-title-wrap")
    title_tag = title_wrap.select_one("h4") if title_wrap else None
    if title_tag is None:
        raise SkkuParseError("Could not parse SKKU SW post title")
...
def _parse_date_from_info(info_texts: list[str]) -> tuple[date, int]:
    ...
    raise SkkuParseError("Could not parse SKKU SW post date")
...
def _parse_description(box: Tag) -> str:
    content = _find_description_content(box)
    if content is None:
        raise SkkuParseError("Could not find SKKU SW post description")
    ...
    if not description:
        raise SkkuParseError("Could not parse SKKU SW post description")
